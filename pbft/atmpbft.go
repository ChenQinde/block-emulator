...
type AtmPbft struct {
	...
	txlog        *csv.Writer
	queuelog     *csv.Writer
	shardNums    int  // shard的数目
	isMainNode   bool // 判断是否为主节点
	PartitionMap map[string]int
}

func NewAtmPbft(shardID int, nodeID int) *AtmPbft {
	...
	p.shardNums = config.Shard_num
	p.isMainNode = p.Node.nodeID == "N0"
	p.PartitionMap = make(map[string]int)
	return p
}

func NewatmLog(p *AtmPbft) {
...
func (p *AtmPbft) handleRequest(data []byte) {
	//切割消息，根据消息命令调用不同的功能
...
// 处理预准备消息
func (p *AtmPbft) handlePrePrepare(content []byte) {
...
// 处理准备消息
func (p *AtmPbft) handlePrepare(content []byte) {
	//使用json解析出Prepare结构体
	pre := new(Prepare)
...
// 处理提交确认消息
func (p *AtmPbft) handleCommit(content []byte) {
	//使用json解析出Commit结构体
	c := new(Commit)
...
func (p *AtmPbft) requestBlocks(startID, endID int) {
	r := RequestBlocks{
		StartID:  startID,
		EndID:    endID,
		ServerID: "N0",
...
func (p *AtmPbft) handleRequestBlock(content []byte) {
...
func (p *AtmPbft) SendBlocks(rb *RequestBlocks, blocks []*core.Block) {
	s := SendBlocks{
		StartID: rb.StartID,
		EndID:   rb.EndID,
		Blocks:  blocks,
...
func (p *AtmPbft) handleSendBlock(content []byte) {
	sb := new(SendBlocks)
...
// 向除自己外的其他节点进行广播
func (p *AtmPbft) broadcast(cmd command, content []byte) {
...
// 为多重映射开辟赋值
func (p *AtmPbft) setPrePareConfirmMap(val, val2 string, b bool) {
...
// 为多重映射开辟赋值
func (p *AtmPbft) setCommitConfirmMap(val, val2 string, b bool) {
...
func (p *AtmPbft) handleRelay(content []byte) {
	relay := new(Relay)
